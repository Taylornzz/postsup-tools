...
export const VERSION = "v2.17.6";
const CHANGELOG = [
  "v2.17.6 — Added the ARRI ALEXA 35 Live (the multicam live body, same 4.6K Super 35 sensor as the ALEXA 35, so it records the same ARRIRAW/ProRes and flows through Storage with ARRI cards). The catalog-watch panel in Capture now has clear 'add' and 'dismiss' buttons on each suggestion: 'add' copies a ready request to paste to Claude (which verifies the specs before adding), 'dismiss' hides it for good. Note: the watch's suggested 'Kinefinity MAVO Edge 8K II' isn't a real product — only the MAVO Edge 8K exists, and it's already in the catalog — so it wasn't added.",
