...
export const NODES: PNode[] = [
  ...
  n("v-late", "vfx", "picture", "vfx-master", "Late VFX Drop-ins", "Finishing reality: finals (or revised shots) land AFTER picture lock, during grade/online. Re-conform ONLY the changed shots, re-QC them, update the line-up sheet, and track the knock-on to the grade and delivery dates. Temp VFX in the cut is replaced shot-by-shot as approved finals arrive — protect the master-version history.", "VFX Producer + Online"),
  // 7 — Conform
  ...
  n("del-dcp", "delivery", "picture", "deliverable", "DCP (theatrical)", "DI master → theatrical P3-D65 trim (often a separate DCP grade on a DCI projector) → XYZ γ2.6 12-bit → JPEG2000 (avg bitrate, 250 Mbit/s hard ceiling) → Op1a MXF per reel → SMPTE-TT subs + Atmos IAB → CPL+PKL+ASSETMAP (hashed, signed) → AES-128 + KDM. ISDCF name e.g. TITLE_FTR-1_F-178_EN-XX_INT-TD_51_4K_STUDIO_DATE_FACILITY_OV. Full-pass screen on a calibrated DCI projector before sign-off. OV + required VF packages built.", "DCP Mastering House"),
  n("del-kdm", "delivery", "picture", "deliverable", "KDM Key Creation", "Per-screen keys for an ENCRYPTED DCP (always for studio theatrical; optional for festivals/tests; never for archive/trailers). Lifecycle: distributor confirms playdate + screen list → cinema exports its PROJECTOR cert (X.509) → mastering house generates a per-screen KDM tied to the CPL UUID + cert thumbprint + validity window (24 hr before first show → 24–48 hr after last) + forensic-mark flags → push via Doremi/Dolby/Deluxe One/GDC portal → cinema imports to TMS; projector decrypts at playback → auto-expires (extension = new KDM). Watch: wrong CPL UUID (re-master = re-issue EVERY KDM), timezone (KDM in UTC vs NZST — add a 12-hr margin each side), wrong/expired projector cert, forensic-flag mismatch (Atmos/HFR), InterOp KDM on a SMPTE DCP. Dry-run a short-window test KDM on your own QC server first.", "Mastering House + Distributor"),
  ...
  n("del-screener", "delivery", "picture", "deliverable", "Security · TPN · Screeners", "Content-security regime. Vendors must be MPA TPN-assessed — Blue (self-report) → Silver (third-party, the common studio minimum) → Gold (on-site, incl. DCDM/cloud) → Gold Shield (annually re-assessed, accepted by all majors) — against the MPA CSBP (v5.3.1: site security, network segmentation, locked-down workstations, EDR/DLP, MFA/SSO, media handling, cloud key mgmt, incident response). Netflix overlays its own program: TPN Silver+, a named security contact, 24-hr incident reporting, and a per-user forensic watermark (NexGuard / TraceMark) on every screener & cloud-review asset. Post-super owns the vendor security register (tier + expiry) and screener hygiene — never an unwatermarked link, never a shared login, revoke after the window, audit every issue, click-through NDA.", "Post Supervisor"),
...
export const EDGES: PEdge[] = [
  ...
  { from: "v-late", to: "c-timeline", op: "transform", label: "Late finals re-conformed shot-by-shot — re-QC + re-grade only the changed shots", dashed: true },
  ...
  { from: "del-dcp", to: "del-kdm", op: "wrap", label: "Encrypted CPL → per-screen KDM (cert + validity window)" },
  ...
  { from: "del-imf", to: "dp-fonts", op: "annotate", label: "Font licences + embedding rights for titles/subs" },
